...
public class Stack {
    ...
    private void popBottom(Node ptr) {
        if (top == null) {
        ...
        if (ptr.nextNode.nextNode == null) {
            ...
            popBottom(ptr.nextNode);
        }
    }

    /* recursive method how many nodes does the stack count */

    int length() {
        return length(top);
    }

    private int length(Node ptr) {
        if (ptr == null) {
            return 0;
        } else if (ptr.nextNode == null){
            return 1;
